...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    check_csrf();
    $stmt = db()->prepare('SELECT id,`user`,nome,senha_crip,tipo_user FROM usuario WHERE `user`=? LIMIT 1');
...
<form class="card login-card" method="post"><input type="hidden" name="csrf" value="<?=csrf()?>"><span class="eyebrow">ACESSO AO PORTAL</span><h2>Boas-vindas</h2><p class="muted">Entre com suas credenciais para continuar.</p><?php if($error):?><div class="alert error"><?=e($error)?></div><?php endif;?><label>CPF ou usuário<input name="user" required placeholder="Digite seu CPF ou usuário"></label><label>Senha<input type="password" name="senha" required placeholder="Sua senha"></label><button class="btn primary" type="submit">Entrar no portal →</button><a class="btn secondary guest-entry" href="cadastro-convidado.php">Criar acesso como convidado</a></form></section>
<?php page_bottom(); ?>
