...
themeToggle?.addEventListener('click',()=>{const theme=document.documentElement.dataset.theme==='dark'?'light':'dark',root=document.documentElement;root.classList.add('theme-changing');requestAnimationFrame(()=>{applyTheme(theme);root.dataset.themeDirection=theme;try{localStorage.setItem('agenda-theme',theme)}catch(e){}setTimeout(()=>{root.classList.remove('theme-changing');delete root.dataset.themeDirection},1600)});});
...
if(deadlineField){
 ...
 const updateDeadline=()=>{if(!deadlineField.value){deadline.textContent='';deadline.className='deadline-hint';return;}const selected=new Date(`${deadlineField.value}T00:00:00`),today=new Date();today.setHours(0,0,0,0);const days=Math.round((selected-today)/86400000);deadline.textContent=days>=90?`✓ Prazo atendido: ${days} dias de antecedência.`:'⚠ A solicitação deverá ser feita com 90 dias de antecedência.';deadline.className=`deadline-hint ${days>=90?'ok':'warning'}`;};
...
if(picker&&window.L){
 // Divinópolis é somente o foco inicial; o organizador pode navegar livremente.
 const map=L.map(picker).setView([-20.13917,-44.88806],14);
 ...
 let marks=[],line,dots=[],routeRequest,routeCalculation=0;
 ...
 const reset=()=>{routeRequest?.abort();routeRequest=null;routeCalculation++;marks.forEach(m=>map.removeLayer(m));marks=[];clearDrawing();updateEndpoints();status.textContent='Clique para marcar a largada e depois os pontos do percurso.';status.className='availability';};
 ...
 const calculate=async()=>{if(marks.length<2)return;clearDrawing();routeRequest?.abort();const calculation=++routeCalculation;routeRequest=new AbortController();const timeout=setTimeout(()=>routeRequest.abort(),25000);status.textContent=`Calculando percurso para corrida com ${marks.length-2} ponto(s) de passagem...`;
  // Cada trecho é consultado nos dois sentidos. O menor resultado é usado e,
  // quando veio no sentido inverso, sua geometria é revertida. Isso mantém o
  // traçado sobre ruas e rodovias sem impor mão única a uma corrida interditada.
  const positions=marks.map(marker=>marker.getLatLng());
  const fetchSegment=async(start,end)=>{const coordinates=`${start.lng},${start.lat};${end.lng},${end.lat}`;const url=`https://router.project-osrm.org/route/v1/driving/${coordinates}?overview=full&geometries=geojson&steps=false&continue_straight=false`;const response=await fetch(url,{signal:routeRequest.signal});const data=await response.json().catch(()=>null);if(!response.ok||data?.code!=='Ok'||!data.routes?.[0]?.geometry?.coordinates?.length)return null;return data.routes[0]};
  try{const segments=[];for(let i=1;i<positions.length;i++){const [forward,reverse]=await Promise.all([fetchSegment(positions[i-1],positions[i]),fetchSegment(positions[i],positions[i-1])]);if(!forward&&!reverse)throw new Error('route_not_found');if(!reverse||forward&&forward.distance<=reverse.distance)segments.push({route:forward,reversed:false});else segments.push({route:reverse,reversed:true});}if(calculation!==routeCalculation)return;let distance=0,points=[];segments.forEach((segment,index)=>{distance+=segment.route.distance;let part=segment.route.geometry.coordinates.map(([lon,lat])=>[lat,lon]);if(segment.reversed)part.reverse();if(index)part=part.slice(1);points.push(...part)});line=L.polyline(points,{color:'#24a5b8',weight:6}).addTo(map);drawDots(points);routeJson.value=JSON.stringify(points);status.textContent=`✓ Percurso da corrida: ${(distance/1000).toFixed(2)} km, ${marks.length} marcações. Sentidos das vias ignorados.`;status.className='availability ok';}catch(e){if(e.name==='AbortError'&&calculation!==routeCalculation)return;status.textContent=e.name==='AbortError'?'O serviço de rotas demorou para responder. Tente novamente.':'Não foi possível ligar esses pontos pelas vias mapeadas. Adicione um ponto de passagem sobre a estrada desejada.';status.className='availability no';}finally{clearTimeout(timeout);if(calculation===routeCalculation)routeRequest=null;}}
 ...
 document.querySelector('#undoRoute')?.addEventListener('click',()=>{const marker=marks.pop();if(marker)map.removeLayer(marker);clearDrawing();updateEndpoints();if(marks.length>1)calculate();else{routeRequest?.abort();routeRequest=null;routeCalculation++;}});
