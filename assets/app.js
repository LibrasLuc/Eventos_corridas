...
if(picker&&window.L){
 ...
 const calculate=async()=>{if(marks.length<2)return;clearDrawing();routeRequest?.abort();const calculation=++routeCalculation;routeRequest=new AbortController();const timeout=setTimeout(()=>routeRequest.abort(),20000);status.textContent=`Calculando percurso para corrida com ${marks.length-2} ponto(s) de passagem...`;
  // OSRM usa o mapa viário completo, incluindo rodovias. As coordenadas devem
  // ser enviadas na ordem longitude,latitude, diferente da ordem usada no Leaflet.
  const coordinates=marks.map(marker=>{const point=marker.getLatLng();return`${point.lng},${point.lat}`}).join(';');
  ...
  try{const response=await fetch(url,{signal:routeRequest.signal});const data=await response.json().catch(()=>null);if(!response.ok||data?.code!=='Ok'||!data.routes?.[0]?.geometry?.coordinates?.length)throw new Error('route_not_found');if(calculation!==routeCalculation)return;const route=data.routes[0];const points=route.geometry.coordinates.map(([lon,lat])=>[lat,lon]);line=L.polyline(points,{color:'#24a5b8',weight:6}).addTo(map);drawDots(points);routeJson.value=JSON.stringify(points);status.textContent=`✓ Percurso viário: ${(route.distance/1000).toFixed(2)} km, ${marks.length} marcações. Ruas e rodovias habilitadas.`;status.className='availability ok';}catch(e){if(e.name==='AbortError'&&calculation!==routeCalculation)return;status.textContent=e.name==='AbortError'?'O serviço de rotas demorou para responder. Tente novamente.':'Não foi possível ligar esses pontos pelas vias mapeadas. Adicione um ponto de passagem sobre a estrada desejada.';status.className='availability no';}finally{clearTimeout(timeout);if(calculation===routeCalculation)routeRequest=null;}}
