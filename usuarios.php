...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        if($action==='create'){
            ...
            db()->prepare('INSERT INTO usuario(`user`,senha_crip,tipo_user) VALUES(?,?,?)')->execute([$username,password_hash($password,PASSWORD_DEFAULT),$role]);
...
$users=db()->query('SELECT id,`user`,tipo_user FROM usuario ORDER BY FIELD(tipo_user,\'admin\',\'organizador\',\'convidado\'),`user`')->fetchAll();
page_top('Usuários');
?>
...
<section class="card form-section"><div class="section-number">+</div><h2>Criar usuário</h2><form method="post" class="admin-form"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="create"><label>Usuário<input name="user" required minlength="3" placeholder="nome.usuario"></label><label>Senha inicial<input type="password" name="senha" required minlength="8" placeholder="Mínimo 8 caracteres"></label><label>Poder<select name="tipo_user"><option value="convidado">Convidado</option><option value="organizador">Organizador</option><option value="admin">Super Admin</option></select></label><button class="btn primary">Criar usuário</button></form></section>
<div class="user-list">
<?php foreach($users as $user):$roleLabel=match($user['tipo_user']){'admin'=>'Super Admin','organizador'=>'Organizador',default=>'Convidado'};?>
<article class="card user-row"><div><span class="badge <?=$user['tipo_user']==='admin'?'aprovado':''?>"><?=e($roleLabel)?></span><h3><?=e($user['user'])?></h3><?php if($user['id']===$_SESSION['user']['id']):?><small>Sua conta</small><?php endif;?></div><div class="user-actions">
