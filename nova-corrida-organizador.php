            ...
            try {
                ...
                $insert = $pdo->prepare(
                    ...
                    (protocolo, usuario_id, nome_evento, local_ini, local_fin, categoria, desc_corrida,
                     organizador, trajeto_json, percurso_km, dia_ini, dia_fin, status, retorno)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'aprovada', ?)"
                );
                $insert->execute([
                    ...
                    $draft['dia_fin'], 'Cadastrada e revisada pelo organizador.',
                ]);
...
<section class="card event-detail review-document">
    ...
    <dl><dt>Categoria</dt><dd><?= e($draft['categoria']) ?></dd><dt>Período</dt><dd><?= date('d/m/Y', strtotime($draft['dia_ini'])) ?> a <?= date('d/m/Y', strtotime($draft['dia_fin'])) ?></dd><dt>Organizador</dt><dd><?= e($draft['organizador']) ?></dd><dt>Descrição</dt><dd><?= nl2br(e($draft['desc_corrida'])) ?></dd><dt>Largada</dt><dd><?= e($draft['local_ini']) ?></dd><dt>Chegada</dt><dd><?= e($draft['local_fin']) ?></dd></dl>
    <div id="routeView" class="map" data-start="<?= e($draft['local_ini']) ?>" data-end="<?= e($draft['local_fin']) ?>"></div>
    <script type="application/json" id="savedRoute"><?= str_replace('</', '<\/', $draft['trajeto_json']) ?></script>
